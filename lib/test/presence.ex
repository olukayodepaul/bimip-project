...
#       :ets.insert(buf, {shard_offset, {offset, record}})
#       {:ok, offset}
#     end
#   end
...
#       fn {shard_offset, {user_offset, rec}}, {b_acc, i_acc, curr_idx, curr_phys, upd, l_map} ->
#         # Now shard_offset is the global ID we fetched with
#         # and user_offset is the per-user sequence

#         {bin_packet, p_size} = encode_packet(rec, user_offset, state)
...
#             # Note: using user_offset here for the index
#             idx_entry = <<byte_size(u_bin)::16, u_bin::binary, rec.p::32, user_offset::64, state.active_base::64, curr_phys::64>>
#             :ets.insert(idx_cache(state.shard), {{rec.u, rec.p, user_offset}, {state.active_base, curr_phys}})
#             {[i_acc | idx_entry], [{rec.u, user_offset} | upd]}
#           else
#             {i_acc, upd}
#           end

#         # Continue reducing
#         {[b_acc | bin_packet], new_i_acc, curr_idx + 1, curr_phys + p_size, new_upd, Map.put(l_map, rec.u, user_offset)}
#       end
...
#     # 1. Fetch current boundaries
#     # Using lookup_element for speed since we know the keys exist
...
#     # 2. Calculate the range for this specific recursive step
#     start_idx = last_ptr + 1
#     end_idx = min(start_idx + @stable_limit - 1, current_head)

#     # 3. Fetch sequentially and handle "gaps" from incomplete writes
...
#             [{^i, data}] ->
#               # Delete immediately as we fetch
#               :ets.delete(buf, i)
#               {:cont, {[ {i, data} | acc], i}}
#             [] ->
#               # Stop if we hit a message that hasn't been fully inserted yet
#               {:halt, {acc, i - 1}}
#           end
#         end)
#       end

#     # 4. Determine result based on whether we found messages
#     result =
#       if items == [] do
#         :"$end_of_table"
#       else
#         # Move the pointer forward so the next recursive call or next flush cycle starts here
#         :ets.insert(u_offsets, {{:last_shard_offset, state.shard}, last_processed_idx})
#         {Enum.reverse(items), :continue}
#       end
...
#         # Write batch to disk via your existing logic
#         new_state = process_batch(state, items)

#         # Check if more data arrived or remains
