...
defp recover_counters_from_anchor(_shard, bin_path) do
  ...
  def handle_call({:fetch, user, p, device_id, batch_size}, _from, state) do
    # ✅ Bookmark now only returns {seg_id, log_off}
    {seg_id, log_off} = Queue.DeviceBookmark.get(device_id, user, p)

    # Calculate the logical gate for the Sparse Index jump
    gate_off = if log_off > 0, do: log_off - rem(log_off - 1, @user_stride), else: 0

    # Resolve logical offset to a physical position using the Shard's Sparse Index
    {actual_seg, actual_phys} = case :ets.lookup(idx_cache(state.shard), {user, p, gate_off}) do
      [{_, {s, pos}}] -> {s, pos}
      _ -> {seg_id, 0} # Fallback: Start of the segment
    end
      ...
      items ->
        ...
        {io_list, final_pos, final_msg_count, final_state} =
          Enum.reduce(sorted, {[], state.current_size, state.msg_count, state}, fn {_s, off, rec}, {acc_io, curr_p, acc_c, acc_s} ->
            ...
            # ✅ Removed curr_p from Initializer
            if off == 1, do: Queue.DeviceBookmark.mark_initializer(rec.u, rec.p, {rec.s, acc_s.active_base, off})

            {packet, p_size, updated_s} = build_packet_data(acc_s, rec, off, curr_p)

            # ✅ Removed curr_p from Anchor
            Queue.DeviceBookmark.mark_anchor(rec.u, rec.p, updated_s.active_base, off)
