defmodule Queue.QueueLogImpl do
  ...
  def write(fd, partition_id, user, to, payload, message_id) do
  ...
  def safe_write(fd, partition_id, user, to, payload, message_id, attempt \\ 1) do
    case write(fd, partition_id, user, to, payload, message_id) do
      {:ok, offset, status} ->
      ...
      {:error, reason} when attempt < @write_max_retries and transient_error?(reason) ->
        :timer.sleep(100 * attempt)
        safe_write(fd, partition_id, user, to, payload, message_id, attempt + 1)
  ...
  def fetch(user, device_id, partition_id, limit \\ 10) when limit > 0 do
    ...
    with :ok <- ensure_files_exist(user, partition_id),
      ...
      {messages_reversed, final_seg, final_pos} =
        ...
        |> Enum.reduce_while({[], start_seg, start_pos}, fn seg, {acc, _, _} ->
      ...
      {:ok,
       %{
         messages: Enum.map(Enum.reverse(messages_reversed), &wrap_message(&1, user, device_id)),
